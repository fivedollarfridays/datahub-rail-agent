...
Echo the AC checklist back (one line per criterion) before starting work, so
it is visible up front what "done" means for this task.

**Engage enforcement:** an engage-driven completion now runs the SAME
strict AC gate as an interactive `task update --status done` — it is not
advisory. An unchecked criterion at completion time blocks the task (lands
`blocked`, not `done`) instead of just printing a warning. Check boxes off
WITH evidence as you satisfy them (see "Check Off Acceptance Criteria"
below) rather than leaving them for a final sweep.
...
bpsai-pair ttask done TRELLO-XX --no-strict --summary "<summary>"
```

For an entire engage run (not one task), the lane-level equivalent is
`bpsai-pair engage <backlog> --allow-unverified-ac` — only with explicit
user approval, same as `--no-strict`.
