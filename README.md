...
Self-hosted DataHub models the Assertion entity and ingests results from dbt
and Great Expectations, yet ships no evaluator, so an OSS team can store
assertion results with no way to produce them. This agent is that evaluator.
It runs against a plain open-source quickstart using only the MCP Server and
the public OpenAPI, with no Cloud dependency, and then does three things
assertions structurally do not:

1. **The verdict becomes context other agents inherit.** Verdicts are written
   back onto the dataset as a searchable `rail.status.*` tag plus structured
   properties. Any other MCP-reading agent already calling `get_entities` or
   `search` sees rail's health verdict on assets it was about to touch,
   without knowing this project exists. Assertions live in a monitoring
   product; these live in the context platform.
   [Details](#write-back-findings-live-in-the-graph)
2. **The monitor proves its own liveness from capture evidence.**
   `check-monitor` exits non-zero when the newest `rail.last_run` stamp *in
   the graph* is stale, or absent entirely, because no evidence is a blind
   state rather than a passing one. Never a heartbeat file.
3. **Triage separates a stalled producer from a broken delivery.** A stale
   dataset whose immediate upstreams are all fresh is a different fault, and
   the report says so instead of blaming a job whose history reads green.

**No model is in the fact path.** The dependency list is `mcp`, `aiohttp`,
and `PyYAML`. Every fact in every incident report is a graph read rendered
deterministically, and each report ends with a provenance table naming the
tool, entity and read timestamp behind every claim. Nothing is phrased by a
model, so nothing can be hallucinated.

**Zero-install, 90 seconds.** Everything below is committed output from a real
two-run pass, regenerated by `scripts/refresh_sample_outputs.py` rather than
written by hand:

- [`sample-outputs/incident_orders_archive.md`](sample-outputs/incident_orders_archive.md)
  — an owner-addressed incident report ending in the provenance table
- [`sample-outputs/state_digest.txt`](sample-outputs/state_digest.txt) — the
  day-2 digest, the same three faults now `CHRONIC` instead of `NEW`
- [`sample-outputs/schema_drift_transactions_warehouse.diff`](sample-outputs/schema_drift_transactions_warehouse.diff)
  — a generated patch that `git apply --check` accepts, verified in CI
- [the write-back screenshot](docs/img/writeback-ui.png) — the verdict living
  on the dataset page inside DataHub

**Verify the test claim in one command**, no DataHub required, because the
suite blocks sockets for every test with a canary that proves the block:

```bash
uv venv --python 3.11 && uv pip install -e ".[dev]" -c constraints.txt
python -m pytest tests/ -q      # 220 passed, 90% line coverage
```

**Open-source contribution:** a health-monitoring agent patterns guide
contributed upstream to the MCP Server itself,
[acryldata/mcp-server-datahub#174](https://github.com/acryldata/mcp-server-datahub/pull/174).
...
(server v1.7.0, mcp-server-datahub v3.4.6) and the output shown is the
output it produced.
...
Graph reads go through the DataHub MCP Server (v3.4.6), using the tools it
actually exposes:
